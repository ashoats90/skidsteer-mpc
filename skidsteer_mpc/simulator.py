...
class SimLog:
    ...
    def min_clearance(self) -> float:
        if self.obstacle_xy is not None:
            dx = self.z[:, 0] - self.obstacle_xy[:, 0]
            dy = self.z[:, 1] - self.obstacle_xy[:, 1]
...
class ClosedLoopSimulator:
    ...
    def run(self) -> SimLog:
        ...
        cols = {k: [] for k in ("t", "z", "u", "ref", "mode", "fault", "buffered", "solve_ms", "obstacle_xy")}
        for i in range(steps):
            t_now = i * c.dt
            if hasattr(self.obs, "update"):
                self.obs.update(t_now)
            ref_preview = self.ref.preview(t_now, c.horizon, c.dt)
            ...
            if self.safety_filter is not None:
                res = self.safety_filter(z, res, t_now)
            ...
            cols["t"].append(t_now); cols["z"].append(z.copy()); cols["u"].append(u)
            ...
            cols["obstacle_xy"].append([self.obs.spec.x, self.obs.spec.y])
